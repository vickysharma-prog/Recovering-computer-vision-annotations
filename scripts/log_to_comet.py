...
def make_view(project: str, workspace: str | None) -> None:
    """Save a project view whose panels point at the runs just written.

    Comet's default view auto-generates a panel per metric and ends up showing
    "no data for this chart" once metric names change. This pins three panels
    that are known to have data. It has to run after the upload, because each
    --clean mints new experiment keys and a view holds those keys.

    The API offers create and read but no update, so this adds a view rather
    than replacing one; the newest is the correct one.
    """
    import json
    ...
    api = API()
    ws = workspace or api.get_default_workspace()
    ids = {e.get_name(): e.id for e in api.get(ws, project)}
    det = [v for k, v in ids.items() if k.startswith("detection")]
    cls = [v for k, v in ids.items() if k.startswith("classification")]
    timeline = next((v for k, v in ids.items() if "timeline" in k), None)
    if not (det and cls and timeline):
        print("  skipped view: expected runs are missing")
        return

    def line(metric, title, keys):
        return {"chartId": metric, "chartName": title, "chartType": "line",
                "metricNames": [metric], "experimentKeys": keys,
                "selectedXAxis": "step", "hidden": False}

    charts = {
        "line": [
            line("detection_ratio",
                 "Detection: dots found / dots present "
                 "(0 sparse, 1 medium, 2 dense)", det),
            line("classification_agreement",
                 "Classification agreement by density band", cls),
        ],
        "image": [{"chartId": "progress", "chartName": "Project progress",
                   "chartType": "image", "experimentKeys": [timeline],
                   "imageNames": ["00_project_progress"], "hidden": False}],
        "numberOfCharts": 3,
    }
    for empty in ("scatter", "bar", "parallel", "scalar", "data", "custom",
                  "video", "pcd", "curves", "python", "autoGeneratedMetrics",
                  "autoGeneratedCurve", "autoGeneratedImage",
                  "autoGeneratedDataframe"):
        charts[empty] = []

    view = View(name="Project overview",
                chart_state=json.dumps({"charts": charts, "layout": None}))
    created = api.create_view(ws, project, view)
    if created is not None:
        print(f"  view: comet.com/{ws}/{project}/view/{created.template_id}")
...
def main() -> int:
    ...
    make_view(args.project, args.workspace)
