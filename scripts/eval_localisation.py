...
def main() -> None:
    ...
    for path in files:
            ...
            try:
                ...
                if dbox is not None:
                    ...
                    try:
                        ...
                        pass
                # Sets `class_name` on each dot in place; `dots` stays in the same
                # order as `pred_xy`, so the match indices still line up.
                assign_classes(dots, entries)
                for i, j, _d in m:
                    want = true_cls[j]
                    if not want:
                        continue
                    have = getattr(dots[i], "class_name", None) if i < len(dots) else None
                    cls_n += 1
                    if (have or "").strip() == want.strip():
                        cls_ok += 1
                    else:
                        confusion[(want.strip(), (have or "none").strip())] += 1
        ...
        print(f"  {lab['frame'][:32]:34s} {path_used:8s} labels={len(true_xy):5d} "
              ...
              f"err={err:4.1f}px  swept {swept[0]}/{swept[1]}")
    ...
    if df.cls_scored.sum():
        acc = df.cls_correct.sum() / df.cls_scored.sum()
        print(f"\nCLASSIFICATION on {int(df.cls_scored.sum())} matched, class-labelled dots")
        print(f"  per-dot accuracy {acc:.3f}")
